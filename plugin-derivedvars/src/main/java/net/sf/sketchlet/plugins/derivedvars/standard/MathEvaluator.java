...
public class MathEvaluator {
    ...
    Hashtable<String, String> usedVariables = new Hashtable<String, String>();
    ...
    private static Double evaluateExpression(Operator o, Double f1, Double f2) {
        ...
        if ("+".equals(op)) {
            ...
            res = new Double(Math.cos(Math.toRadians(f1.doubleValue())));
        } else if ("sin".equals(op)) {
            res = new Double(Math.sin(Math.toRadians(f1.doubleValue())));
        } else if ("tan".equals(op)) {
            res = new Double(Math.tan(Math.toRadians(f1.doubleValue())));
        } else if ("acos".equals(op)) {
            res = new Double(Math.toDegrees(Math.acos(f1.doubleValue())));
        } else if ("asin".equals(op)) {
            res = new Double(Math.toDegrees(Math.asin(f1.doubleValue())));
        } else if ("atan".equals(op)) {
            res = new Double(Math.toDegrees(Math.atan(f1.doubleValue())));
    ...
    public void help(JFrame frame) {
        ...
        for (int i = 0; i < operators.length; i++) {
            String strOper = "";
            if (operators[i].type == 2) {
                strOper = "a " + operators[i].getOperator() + " b";
            } else if (operators[i].type == 1) {
                strOper = operators[i].getOperator() + "(x)";
            }
    ...
    public Double getVariable(String s) {
        char c = s.charAt(0);
        if ((c >= 'z' || c < 'a') || s.contains("+") || s.contains("-") || s.contains("*") || s.contains("/") || s.contains("^") || s.contains("%") || s.contains("(") || s.contains(")")) {
            return null;
        }
        try {
            // return (Double) variables.get(s);
            usedVariables.put(s, s);
            return new Double(VariablesBlackboardContext.getInstance().getVariableValue(s));
        } catch (Exception e) {
        }
