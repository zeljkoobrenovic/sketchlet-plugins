...
public class WidgetGraphvizDotMindMap extends WidgetGraphvizDot {
    ...
    // @WidgetPluginProperty(name = "dot parameters", initValue="", description = "Additional parameters to be sent to the dot program")
    // private String cmdLineParams = "";
    public WidgetGraphvizDotMindMap(ActiveRegionContext region) {
        super(region);
    }
    @WidgetPluginProperty(name = "dot parameters", initValue = "", description = "Additional parameters to be sent to the dot program",
    valueList = {"-Granksep=0"})
    private String cmdLineParams = "";
    ...
    @WidgetPluginProperty(name = "style", initValue = "filled", description = "The style of the shape",
    valueList = {"filled", "outlined"})
    private String style = "filled";
    @WidgetPluginProperty(name = "direction", initValue = "LR", description = "LR, RL, TB, BT",
    valueList = {"LR", "RL", "TB", "BT"})
    private String rankdir = "LR";
    @WidgetPluginProperty(name = "splines", initValue = "true", description = "Draw endges with splines",
    ...
    @WidgetPluginProperty(name = "node separation", initValue = "0.1", description = "Minimum space between two adjacent nodes in the same rank, in inches.",
    valueList = {"0", "0.05", "0.1", "0.15", "0.2", "0.25", "0.3", "0.35", "0.4", "0.45", "0.5"})
    ...
    protected String prepareDot(String text) {
        ...
        strDot += "graph [\n";
        strDot += "	splines=" + splines + ",\n";
        strDot += "	rankdir=\"" + rankdir + "\",\n";
        strDot += "	nodesep=" + nodesep + "\n";
        strDot += "]\n";
        strDot += "node [\n";
        strDot += "    shape = " + shape + ",\n";
        strDot += "    height = auto,\n";
        strDot += "    fontsize = " + fontsize + ",\n";
        strDot += "    color = " + color + ",\n";
        strDot += "    style = " + style + ",\n";
        strDot += "    fontname = \"" + fontName + "\" ];\n";
        strDot += "edge [\n";
        strDot += "    arrowsize = " + arrowsize + ",\n";
        strDot += "    fontsize = " + fontsize + ",\n";
        strDot += "    fontname = \"" + fontName + "\" ];\n";
        int prevLevel = 0;
        String prevLine = "";
        ...
        for (String line : lines) {
            ...
            if (n1 > 0) {
                ...
                if (n1 >= 0 && n2 > n1) {
                    ...
                    strDot += "    \"" + line.substring(0, n1).trim() + "\" [" + settings.get(level) + "];\n";
                }
            } else {
                if (settings.size() > 0) {
